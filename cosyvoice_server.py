...
def async_generate_audio_stream(
        text: str,
        voice_id: str = None,
...
async def inference_sft(
        ...
        seed: Optional[int] = Form(default=0, description="自定义随机种子")
):
    """
    流式 TTS 接口
...
async def tts_zero_shot(
    ...
    def stream_generator():
        for chunk in generate_audio_stream(text, prompt_text, prompt_wav_data, stream=True, seed=seed):
            yield chunk
